...
interface EventHandlerMap {
  [key: EventType]: EventHandler<unknown>[];
}
...
export class EventBus extends Singleton<EventBus>() {
   ...
   * @returns this 返回当前实例以支持链式调用 / Returns this instance for method chaining
   */
  once<T = unknown>(event: EventType, handler: EventHandler<T>): this {
  ...
  off<T = unknown>(event: EventType, handler?: EventHandler<T>): this {
    ...
    if (handlers) {
      this.handlers[event] = handlers.filter(
        (h) => h !== (handler as EventHandler<unknown>)
      );
   ...
   * @returns this 返回当前实例以支持链式调用 / Returns this instance for method chaining
   */
  emit<T = unknown>(event: EventType, payload?: T): this {
    const handlers = this.handlers[event];
    if (handlers) {
